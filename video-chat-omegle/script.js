window.addEventListener('DOMContentLoaded', () => {
  ...
  // ---------------------- دالة عرض الإعلان (بالترتيب المتسلسل) ----------------------
  function playAdVideo() {
    ...
    currentAdIndex = (currentAdIndex + 1) % adVideosList.length; // ترتيب متسلسل
    ...
    const adTimeout = setSafeTimer(hideAdVideo, 50000); // احتياطي 50 ثانية
  ...
  async function startSearch() {
    ...
    if (!mediaReady) {
      // إذا فشل initMedia، فهو يعيد المحاولة بنفسه، لا نحتاج إلى فعل شيء هنا
      return;
    }
    cleanupConnection();
  ...
  // ---------------------- MEDIA INIT مع إعادة المحاولة التلقائية ----------------------
  ...
  // باقي الكود بدون تغيير (socket events, WebRTC, إلخ)...
  // (تم ترك باقي الكود كما هو لأنه لم يُطلب تعديله)
  ...
  // ---------------------- WEBRTC و KeepAlive و Stats و Exit ----------------------
  // (باقي الدوال كما هي: createPeerConnection, setupKeepAliveChannel, startPingLoop, startStatsMonitor, exitBtn.onclick ...)
