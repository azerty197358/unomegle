// Socket.io connection handling with infinite reconnection attempts
const socket = io();
...
// Reconnection state - removed MAX_RECONNECT_ATTEMPTS limit
...
// Create blurred background canvas for remote video
const remoteVideoBlur = document.createElement('canvas');
remoteVideoBlur.id = 'remoteVideoBlur';
remoteVideoBlur.style.cssText = `
  position: absolute;
  top: 0;
  left: 0;
  width: 100%;
...
if (remoteVideo && remoteVideo.parentElement) {
  ...
  remoteVideo.parentElement.appendChild(remoteVideoBlur);
...
function showRemoteSpinnerOnly(show) {
  ...
  if (remoteVideoBlur) remoteVideoBlur.style.display = show ? 'block' : 'none';
}
...
function hideAllSpinners() {
  ...
  if (localVideo) localVideo.style.display = 'block';
  if (remoteVideoBlur) remoteVideoBlur.style.display = 'none';
}
...
function createPeerConnection() {
  ...
  try {
    ...
    peerConnection.ontrack = e => {
      ...
      // Capture first frame for blurred background
      const captureFirstFrame = () => {
        if (remoteVideo.videoWidth > 0 && remoteVideo.videoHeight > 0) {
          remoteVideoBlur.width = remoteVideo.videoWidth;
          remoteVideoBlur.height = remoteVideo.videoHeight;
          const ctx = remoteVideoBlur.getContext('2d');
      ...
      remoteVideo.onplay = () => {
        clearInterval(captureInterval);
        remoteVideoBlur.style.display = 'none';
      };
...
// Attempt recovery with infinite reconnection attempts
async function attemptRecovery() {
  // Removed MAX_RECONNECT_ATTEMPTS check - infinite reconnection
  reconnectAttempts++;
