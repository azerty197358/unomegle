...
const ALLOWED_IPS = [];  // أصبحت فارغة للسماح للجميع
const ADMIN_PASSWORD = process.env.ADMIN_PASSWORD || "admin123"; // كلمة مرور الأدمن
...
function createAdminSession(ip) {
    ...
    adminSessions.set(sessionId, { ip, expires });
    return sessionId;
}

function validateAdminSession(sessionId, clientIp) {
...
// ====== تطبيق التحقق على صفحة الأدمن فقط ======
app.use((req, res, next) => {
    const clientIp = realIP(req);
    
    // التحقق إذا كان الطلب لصفحة الأدمن
    if (req.path === '/admin' || req.path.startsWith('/admin/')) {
        // التحقق من الجلسة
        const sessionId = req.cookies?.adminSession;
        if (sessionId && validateAdminSession(sessionId, clientIp)) {
            return next();
        }
        
        // إذا لم يكن هناك جلسة صالحة، تحقق من كلمة المرور إذا كانت في POST
        if (req.method === 'POST' && req.path === '/admin/login') {
            return next(); // السماح لطلب تسجيل الدخول
        }
        ...
        // رفض الوصول للمسارات الأخرى بدون مصادقة
        return res.status(401).send('Unauthorized');
    }
    
    // السماح بجميع المسارات الأخرى للجميع
    next();
});
...
app.get("/admin/database-stats", (req, res) => {
    try {
...
app.post("/admin/unblock-country", (req, res) => {
    stmtDeleteBannedCountry.run((req.body.code || "").toUpperCase());
    emitAdminUpdate();
    res.send({ ok: true });
});

app.post("/admin/clear-blocked", (req, res) => {
...
app.post("/unban-fingerprint", (req, res) => {
    unbanUser(null, req.body.fp);
    res.send({ ok: true });
});

app.post("/manual-ban", (req, res) => {
...
app.post("/remove-report", (req, res) => {
    const target = req.body.target;
    if (!target) return res.status(400).send({ error: true });
...
server.listen(PORT, () => {
    ...
    console.log("كلمة مرور الأدمن الافتراضية: " + ADMIN_PASSWORD);
    console.log("يمكن تغييرها عبر متغير البيئة ADMIN_PASSWORD");
});
