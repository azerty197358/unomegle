  ...
  try {
    ...
    let html = `
      ...
      <body>
        ...
        <!-- =============================== -->
        <!-- NEW: broadcast message to all -->
        <!-- =============================== -->
        <h2>Send Message to All Users</h2>
        <form action="/admin-broadcast" method="POST" style="margin-bottom:20px;">
            <textarea name="message" rows="3" style="width:300px;" placeholder="Type message..."></textarea>
            <br><br>
            <button type="submit" style="background:#28a745;color:#fff;padding:8px 12px;border:none;cursor:pointer;">
              Send to All
            </button>
        </form>
        <!-- =============================== -->
...
// =============================================
// NEW ROUTE: Broadcast to all connected clients
// =============================================
...
io.on("connection", (socket) => {
  ...
  if (banExpire && (banExpire === Infinity || banExpire > Date.now())) {
    const banMessage = banExpire === Infinity ?
      'You have been permanently banned for engaging in pornographic activities.' :
      'You have been banned for 24 hours for engaging in pornographic activities.';
  ...
  socket.on("reportPorn", async (payload) => {
    ...
    const reportedIp = reportedSocket
      ? (reportedSocket.handshake.headers['cf-connecting-ip'] ||
         (reportedSocket.handshake.headers['x-forwarded-for']
          ? reportedSocket.handshake.headers['x-forwarded-for'].split(',')[0].trim()
    ...
    try {
      ...
      socket.emit("reportHandled", {
        message: "Error submitting report. Try again.",
        error: true
      });
    }
  });
